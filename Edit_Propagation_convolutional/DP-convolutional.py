...
class DeepProp(object):
    ...
    def recoloring(self, strk_path, out_path="./out.jpg"):
        ...
        for y, row in enumerate(strk[:, :, 3]):
            for x, val in enumerate(row):
                ...
                for k in range(label_dim):
                    # coord_simple_num = int(len(coord_simple[self.label2color[k]]) * 0.7)
                    # perm = numpy.random.permutation(len(coord_simple[self.label2color[k]]))
                    # new_coord_simple = numpy.array(coord_simple[self.label2color[k]], numpy.float32)
                    # new_coord_simple = new_coord_simple[perm[:coord_simple_num]]
        ...
        # training
        t0 = time.time()
        dnn = DNN9.DNN(channel=label_dim)
        ...
        d.addPairwiseGaussian(sxy=3, compat=3)
        d.addPairwiseBilateral(sxy=60, srgb=13, rgbim=img, compat=10)
        q = d.inference(6)
        ...
        for i in range(label_dim):
            cv2.imwrite('data/out' + str(i) + '.tif', p_img[i])
            cv2.imwrite('data/out' + str(i) + 'crf_mask.tif', crf_mask[i])
            cv2.imwrite('data/out' + str(i) + 'crf_img.tif', crf_img[i])
